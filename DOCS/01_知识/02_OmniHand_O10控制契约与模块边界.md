...
1. 厂商 O10 `joint_error_states` 中任一关节任一错误位非零；Phase 1 对 `stalled`、`overheat`、`over_current`、`motor_except` 和 `commu_except` 均采用同一保守口径；
...
错误位恢复为零或通信重新出现均不能自动清除锁存。清除故障必须由操作者显式请求，并且同时满足：当前错误报告全部为零、通信健康、成功独立读取一帧新鲜合法的真实主动关节状态，并以该状态重新初始化硬限制器。完成后：
