...
Launchpad 网页不提供任何操作/授权 Service（自 `4fedfaa` 起 `arm`/`disarm` 已移除，网页亦不暴露 `clear_fault`）。运动由新鲜合法软目标直接驱动：只要反馈就绪、错误监控就绪且目标新鲜，实体手就会运动。因此网页启动成功、ROS topic 有发布者或反馈可读，都不能单独证明实体灵巧手已经安全；戴手套前必须确认实体手周围安全且只连接真机 provider（不要同时点亮合成输入源）。
...
4. `fault_latched` 是否为 false、控制状态是否允许进入下一阶段（`4fedfaa` 起无 `arm`；运动由新鲜目标直接驱动，无独立使能前置检查）；
