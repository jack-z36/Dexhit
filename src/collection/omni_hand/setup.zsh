...
for distro in humble iron jazzy rolling; do
    if [[ -d "$SCRIPT_DIR/$distro" ]] && [[ -f "$SCRIPT_DIR/$distro/setup.zsh" ]]; then
        ...
        echo "[2/4] Sourcing /opt/ros/$distro/setup.zsh..."
        source "/opt/ros/$distro/setup.zsh"
        if [[ $? -ne 0 ]]; then
            echo "Error: Failed to source /opt/ros/$distro/setup.zsh" >&2
            return 1
        fi
        echo "[3/4] Sourcing $SCRIPT_DIR/$distro/setup.zsh..."
        source "$SCRIPT_DIR/$distro/setup.zsh"
        if [[ $? -eq 0 ]]; then
            ...
            echo "Warning: Failed to source $SCRIPT_DIR/$distro/setup.zsh" >&2
        fi
    fi
done
